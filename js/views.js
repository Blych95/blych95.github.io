...
function renderTareas() {
  ...
  const pendientes = DB.tareas.filter((t) => !t.done);
  const completadas = DB.tareas.filter((t) => t.done);
  pendientes.forEach((t) => buckets[quadrantOf(t)].push(t));

  const quad = (key) => {
    const q = QUADRANTS[key];
    const items = buckets[key].map(renderTaskCard).join("");
    return `
      <div class="quadrant" style="--q:${q.color}">
        ...
        <div class="quad-body">${items || `<p class="muted small">Sin tareas pendientes</p>`}</div>
      </div>`;
  };

  // Registro de tareas completadas (más recientes primero)
  const compSorted = [...completadas].sort((a, b) =>
    String(b.completedAt || "").localeCompare(String(a.completedAt || "")));
  const compHtml = compSorted.length
    ? compSorted.map(renderCompletedTaskRow).join("")
    : `<p class="muted small">Aún no completas tareas. Cuando termines una, aparecerá aquí.</p>`;
  ...
  return `
    ...
    <div class="card">
      <h2 class="card-title">Completadas <span class="chip">${completadas.length}</span></h2>
      <p class="muted small">Registro de lo que ya terminaste. Puedes reabrir o eliminar.</p>
      <div class="done-log">${compHtml}</div>
    </div>
  `;
}

function renderCompletedTaskRow(t) {
  const meta = DB.metas.find((m) => m.id === t.metaId);
  return `
    <div class="done-row">
      <button class="check on" data-action="toggle-task" data-id="${t.id}" aria-label="Reabrir tarea">✓</button>
      <div class="done-main">
        <span class="done-title">${esc(t.title)}</span>
        <span class="done-sub">
          ${meta ? `<span class="chip">${esc(meta.title)}</span>` : ""}
          ${t.areaId ? areaTag(t.areaId) : ""}
          ${t.completedAt ? `<span class="muted small">✓ ${formatDate(ymd(t.completedAt))}</span>` : ""}
        </span>
      </div>
...
function renderCalendario() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const dots = `
      ...
      ${hasAchievementsOn(ds) ? `<span class="cal-dot ach" title="Logros del día"></span>` : ""}
      ${hasDeadlinesOn(ds) ? `<span class="cal-dot due" title="Vence algo este día"></span>` : ""}`;
    cells += `
  ...
  const weekAvg = weekMoodAverage(sel);
  const monthAvg = monthMoodAverage(y, m);
  ...
  return `
    ...
    <div class="grid-2 cal-layout">
      <div class="card">
        ...
        <div class="cal-legend">
          ...
          <span><span class="cal-dot ach"></span> logros</span>
          <span><span class="cal-dot due"></span> plazos</span>
        </div>
      </div>
    ...
    <div class="grid-2">
      ${moodAverageCard("Cómo me sentí esta semana", weekAvg)}
      ${moodAverageCard("Cómo me sentí este mes", monthAvg)}
    </div>
...
/* Tarjeta de promedio de ánimo y energía */
function moodAverageCard(title, avg) {
  if (!avg || !avg.count) {
    return `<div class="card">
      <h2 class="card-title">${title}</h2>
      <p class="muted small">Sin registros de ánimo en este periodo.</p>
    </div>`;
  }
  const moodEmoji = MOOD_ICONS[avg.moodKey] || "🙂";
  const moodLabel = MOOD_LABELS[avg.moodKey] || avg.moodKey;
  const energyEmoji = ENERGY_ICONS[avg.energyKey] || "⚡";
  return `
    <div class="card">
      <h2 class="card-title">${title}</h2>
      <div class="avg-row">
        <div class="avg-item">
          <span class="avg-emoji">${moodEmoji}</span>
          <div><b>${capitalize(moodLabel)}</b><br><span class="muted small">ánimo medio (${avg.moodScore.toFixed(1)}/5)</span></div>
        </div>
        <div class="avg-item">
          <span class="avg-emoji">${energyEmoji}</span>
          <div><b>${capitalize(avg.energyKey)}</b><br><span class="muted small">energía media (${avg.energyScore.toFixed(1)}/3)</span></div>
        </div>
      </div>
      <p class="muted small">${avg.count} registro(s) en el periodo.</p>
    </div>`;
}
...
function renderDayDetail(ds) {
  ...
  // Logros del día
  const ach = achievementsOn(ds);
  const achTotal = ach.tasks.length + ach.metas.length + ach.objetivos.length;
  const achHtml = achTotal
    ? `<ul class="day-list">
        ${ach.objetivos.map((o) => `<li><span class="type-pill obj">Objetivo</span> 🏆 ${esc(o.title)}</li>`).join("")}
        ${ach.metas.map((m) => `<li><span class="type-pill meta">Meta</span> 🎉 <button class="link-btn" data-action="open-meta" data-id="${m.id}">${esc(m.title)}</button></li>`).join("")}
        ${ach.tasks.map((t) => `<li><span class="type-pill task">Tarea</span> ✓ ${esc(t.title)}</li>`).join("")}
      </ul>`
    : `<p class="muted small">No completaste nada registrado este día.</p>`;

  // Vencimientos del día
  const due = deadlinesOn(ds);
  const dueTotal = due.tasks.length + due.metas.length + due.objetivos.length;
  const dueHtml = dueTotal
    ? `<ul class="day-list">
        ${due.objetivos.map((o) => `<li><span class="type-pill obj">Objetivo</span> ⏳ ${esc(o.title)}</li>`).join("")}
        ${due.metas.map((m) => `<li><span class="type-pill meta">Meta</span> ⏳ <button class="link-btn" data-action="open-meta" data-id="${m.id}">${esc(m.title)}</button></li>`).join("")}
        ${due.tasks.map((t) => `<li><span class="type-pill task">Tarea</span> ⏳ ${esc(t.title)}</li>`).join("")}
      </ul>`
    : `<p class="muted small">Nada vence este día.</p>`;
    ...
    <div class="card">
      ...
      <div class="grid-2">
        <div>
          ...
          ${habitHtml}
          <h3 class="mini-title">Logros del día</h3>
          ${achHtml}
        </div>
        <div>
          <h3 class="mini-title">Vence este día</h3>
          ${dueHtml}
